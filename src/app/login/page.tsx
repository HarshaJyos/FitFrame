...
function LoginForm() {
    ...
    const redirect = () => router.push(next);
    ...
    const handleEmail = async (ev: React.FormEvent) => {
        ...
        try { await signInWithEmail(email, password); redirect(); }
        ...
        <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem 1.25rem' }}>
            ...
            <div className="card w-full" style={{ maxWidth: 420, padding: '2.5rem' }}>
                ...
                <button onClick={handleGoogle} disabled={loading}
                    style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '0.85rem', borderRadius: 12, border: '1.5px solid var(--border)', background: '#fff', cursor: 'pointer', fontWeight: 600, fontSize: '0.95rem', color: 'var(--text)', marginBottom: '1.25rem', transition: 'border-color 0.15s' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.25rem' }}>
                    ...
                    <span style={{ fontSize: '0.78rem', color: 'var(--text-3)' }}>or sign in with email</span>
                    <div style={{ flex: 1, height: 1, background: 'var(--border)' }} />
                </div>
