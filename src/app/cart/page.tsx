    ...
    return (
        <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
            ...
            <div className="max-w-3xl mx-auto px-5 py-8">
                <h1 style={{ fontSize: '1.6rem', fontWeight: 800, color: 'var(--text)', marginBottom: '1.5rem' }}>Your Cart</h1>
                ...
                {loading ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        {items.map(item => (
                            <div key={item.id} className="card" style={{ padding: '1.25rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                ...
                                <div style={{ textAlign: 'right' }}>
                                    ...
                                    <button onClick={() => handleRemove(item)} style={{ marginTop: 6, fontSize: '0.78rem', color: '#dc2626', background: 'none', border: 'none', cursor: 'pointer' }}>Remove</button>
                                </div>
                            </div>
                        ))}
                        ...
                        <div className="card" style={{ padding: '1.25rem' }}>
                            ...
                            <button onClick={() => setShowPayment(true)} className="btn-primary w-full" style={{ width: '100%', padding: '1rem', fontSize: '1rem', textAlign: 'center' }}>
                                🛍️ Proceed to Pay →
                            </button>
                        </div>
                    </div>
            ...
            <PaymentModal
                ...
                onSuccess={() => { setItems([]); setShowPayment(false); router.push('/account'); }}
                items={orderItems}
                totalAmount={total}
                shirtSize="M"
                pantsSize={'32"'}
